...
function StarRow() {
  return (
    <div className="flex gap-0.5 mb-4">
      {[...Array(5)].map((_, s) => (
        <svg key={s} viewBox="0 0 12 12" className="w-4 h-4 fill-[#1C9FD6]">
...
export default function TestimonialsSlider() {
  ...
  const go = useCallback((delta: 1 | -1) => {
    ...
    setDirection(delta);
    setVirtualIdx((v) => { setPrevVirtual(v); return v + delta; });
    setContentKey((k) => k + 1);
    setTimeout(() => { isAnimating.current = false; setPrevVirtual(null); }, 600);
  }, []);
  ...
  const t = TESTIMONIALS[realIdx];

  const contentAnimClass = prevReal !== null
  ...
  const touchHandlers = {
    ...
    onTouchEnd: (e: React.TouchEvent) => {
      ...
      if (Math.abs(delta) > 48) { delta > 0 ? goNext() : goPrev(); resetTimer(); }
    },
  };
  ...
  return (
    <div>
      <style>{`
        ...
        .content-animate-right { animation: contentEnterRight 0.45s cubic-bezier(0.34,1.56,0.64,1) forwards; }
        .content-animate-left  { animation: contentEnterLeft  0.45s cubic-bezier(0.34,1.56,0.64,1) forwards; }
      `}</style>

      {/* ── MOBILE: single card, full width ── */}
      <div className="md:hidden" {...touchHandlers}>
        <div
          className="bg-white rounded-[20px] p-6 mx-auto"
          style={{ boxShadow: "0 8px 40px rgba(13,30,44,0.12), 0 0 0 1.5px #1C9FD6" }}
        >
          <div key={contentKey} className={contentAnimClass}>
            <StarRow />
            <div className="font-display text-4xl text-[#1C9FD6]/20 leading-none mb-1 select-none">&ldquo;</div>
            <p className="text-sm text-[#2A3A4A] leading-[1.85] mb-5">{t.text}</p>
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-full overflow-hidden flex-shrink-0 bg-[#E8F6FC] ring-2 ring-[#1C9FD6]/20">
        ...
        <div className="flex items-center justify-between mt-5">
          <div className="flex gap-2 items-center">
            {TESTIMONIALS.map((_, i) => (
              <button
                key={i}
                onClick={() => { const d = i - realIdx; if (d !== 0) { go(d > 0 ? 1 : -1); setTimeout(() => setVirtualIdx((v) => v + (d - (d > 0 ? 1 : -1))), 620); resetTimer(); } }}
                aria-label={`Testemunho ${i + 1}`}
                className="rounded-full transition-all duration-300"
                style={{ width: i === realIdx ? 28 : 8, height: 8, background: i === realIdx ? "#1C9FD6" : "#D5E4EE" }}
              />
            ))}
          </div>
          <div className="flex gap-2">
            <button onClick={() => { goPrev(); resetTimer(); }} className="w-11 h-11 rounded-full border border-[#D5E4EE] bg-white flex items-center justify-center text-[#0D1E2C]" aria-label="Anterior">
              <ArrowLeft size={16} />
            </button>
            <button onClick={() => { goNext(); resetTimer(); }} className="w-11 h-11 rounded-full bg-[#1C9FD6] flex items-center justify-center text-white" aria-label="Próximo">
              <ArrowRight size={16} />
            </button>
          </div>
        </div>
      ...
      <div className="hidden md:block">
        ...
        >
          {slots.map((offset) => {
            ...
            const cardContentAnim = isCenter && prevReal !== null
              ? direction === 1 ? "content-animate-right" : "content-animate-left"
              : "";
            ...
            return (
              <div
                key={`${offset}-${dataIdx}`}
                onClick={() => { if (offset !== 0) { offset > 0 ? goNext() : goPrev(); resetTimer(); } }}
                style={{
                  position: "absolute",
                  width: 360,
        ...
        <div className="flex items-center justify-between mt-8">
          <div className="flex gap-2 items-center">
            {TESTIMONIALS.map((_, i) => (
              <button
                key={i}
                onClick={() => { const diff = i - realIdx; if (diff === 0) return; go(diff > 0 ? 1 : -1); setTimeout(() => { setVirtualIdx((v) => v + (diff - (diff > 0 ? 1 : -1))); }, 620); resetTimer(); }}
                aria-label={`Testemunho ${i + 1}`}
          ...
          <div className="flex gap-2">
            <button onClick={() => { goPrev(); resetTimer(); }} className="w-11 h-11 rounded-full border border-[#D5E4EE] bg-white flex items-center justify-center text-[#0D1E2C] hover:bg-[#1C9FD6] hover:border-[#1C9FD6] hover:text-white hover:scale-110 transition-all duration-200" aria-label="Anterior">
              <ArrowLeft size={16} />
            </button>
            <button onClick={() => { goNext(); resetTimer(); }} className="w-11 h-11 rounded-full bg-[#1C9FD6] border border-[#1C9FD6] flex items-center justify-center text-white hover:bg-[#0D7DB5] hover:scale-110 transition-all duration-200" aria-label="Próximo">
              <ArrowRight size={16} />
            </button>
          </div>
        </div>
